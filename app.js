...
function fitFontSize(ctx, text, weight, family, maxSize, minSize, maxWidth) {
  let size = maxSize;
  while (size > minSize) {
...
// ---- Draw a small decorative heart at x,y ----
...
async function generateCertificate() {
  ...
  // Corner hearts
  [[75, 75], [canvas.width - 75, 75], [75, canvas.height - 75], [canvas.width - 75, canvas.height - 75]]
    .forEach(([x, y]) => drawHeart(ctx, x, y, 18, "#C9A24B"));
  ...
  const titleSize = fitFontSize(ctx, CERTIFICATE.title, "600", "Fraunces", 44, 22, canvas.width - 200);
  ctx.font = `600 ${titleSize}px 'Fraunces'`;
  ctx.fillText(CERTIFICATE.title, canvas.width / 2, 150);

  // Small heart divider under the title
  [-24, 0, 24].forEach((dx, i) => drawHeart(ctx, canvas.width / 2 + dx, 178, i === 1 ? 16 : 11, "#D66B7D"));

  ctx.fillStyle = "#2B2320";
  ctx.font = "17px 'Karla'";
  ctx.fillText(CERTIFICATE.recipientLabel, canvas.width / 2, 220);

  ctx.fillStyle = "#D66B7D";
  const nameSize = fitFontSize(ctx, CERTIFICATE.recipientName, "600", "Fraunces", 38, 20, canvas.width - 240);
  ctx.font = `600 ${nameSize}px 'Fraunces'`;
  const nameWidth = ctx.measureText(CERTIFICATE.recipientName).width;
  ctx.fillText(CERTIFICATE.recipientName, canvas.width / 2, 270);
  drawHeart(ctx, canvas.width / 2 - nameWidth / 2 - 28, 267, 18, "#C9A24B");
  drawHeart(ctx, canvas.width / 2 + nameWidth / 2 + 28, 267, 18, "#C9A24B");

  ctx.fillStyle = "#2B2320";
  ctx.font = "16px 'Karla'";
  let nextY = drawWrappedCenteredText(ctx, CERTIFICATE.closing, canvas.width / 2, 320, 700, 24);
